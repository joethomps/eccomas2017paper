...
Find $X$ such that $$J = X^T \Lambda X$$
\end{frame}

\begin{frame}{Solution of the inverse eigenvalue problem}
Lanczos Algorithm\footcite{gladwell1986inverse}
...
\label{eq:lambda}
\end{equation}
and with normalised eigenvectors having non-zero specified values of their first components.
...
Using this algorithm we can find the second transformation $\mathbf{z} = X^T \mathbf{v}$ which leads to the new system:
\begin{equation}
...
\begin{frame}
transformation $\mathbf{x} = M^{\frac{1}{2}} \mathbf{z}$ where $M$ has the form given in Eq. \ref{eq:ms3} leads to the final system:
\begin{equation}
...
\begin{frame}{Correct form of b and c vectors}
