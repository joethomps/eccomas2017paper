...
%Nodes
	...
	\node[actuator]		(act) 				{};
	\node[cart]	(m1)		[right of=act, node distance=1.4cm]			{$m$};
       \node[pt] 	(pt1) 		[right of=m1, node distance=1.4cm] 	{};
	\node[pt] 	(pt2) 		[right of=pt1, node distance=2cm] 	{};
	\node[cart] 	(m3) 		[right of=pt2, node distance=1.4cm] 	{$m$};
	\node[cart] 	(m4) 		[right of=m3, node distance=1.8cm] 	{$m$};
%Connections	
	\draw[spring] (act) -- node[yshift=9pt] (k1){$k$} (m1);
	
	\draw[spring] (m1) -- node[yshift=9pt] (k2) {$k$} (pt1);
	\draw[thick, dashed] (pt1) -- (pt2);
	\draw[spring] (pt2) -- node[yshift=9pt] (k3) {$k$} (m3);
	\draw[spring] (m3) -- node[yshift=9pt] (k4) {$k$} (m4);
	...
	\node	(X3)		[above of=m4, node distance = 20pt]		{$X_n$};
       ...
       \node[tf]	(G1)		[below of=k1, node distance = -3cm]			{$G$};
	\node[tf]	(G2)		[below of=k2, node distance = -3cm]		{$G$};
	\node[tf]	(G3)		[below of=k3, node distance = -3cm]			{$G$};
	\node[tf]	(G4)		[below of=k4, node distance = -3cm]		{$G$};
	\node[tf]	(F)		[right of=G4, yshift=-0.7cm, node distance = 1.8cm]	{$G$};
	 \node[tf]	(H1)		[below of=G1, node distance = 1.4cm]			{$G$};
	\node[tf]	(H2)		[below of=G2, node distance = 1.4cm]		{$G$};
	\node[tf]	(H3)		[below of=G3, node distance = 1.4cm]			{$G$};
	\node[tf]	(H4)		[below of=G4, node distance = 1.4cm]		{$G$};
